...
// Update the analyzeQuery function
exports.analyzeQuery = async (req, res) => {
  const { query, type, subclass, language = 'en' } = req.body;

  // Validate input
  if (!query || !type || !subclass) {
    return res.status(400).json({ 
      success: false, 
      error: "Query, type, and subclass are required fields." 
    });
  }

  try {
    // Enhanced prompt for Gemini API
    const prompt = `As an expert Ethiopian legal advisor specializing in ${type} (${subclass}), analyze:
"${query}"

Provide in ${language === 'am' ? 'Amharic' : 'English'}:
1. Relevant Ethiopian laws (cite as "Article X")
2. Required evidence/documentation
3. Step-by-step legal procedures
